...
// Searching for all for all of the dogs in the dog model
const findAllDogs = (req, res, callback) => {
  Dog.find(callback);
};
...
const hostPage4 = (req, res) => {
  // Function to be called after the results are found
  const callbackFunc = (err, docs) => {
    if (err) return res.json({ err });

    // Have to have the 'page4' or else the render will not work
    return res.render('page4', { allDogs: docs });
  };
  // Performing the search
  findAllDogs(req, res, callbackFunc);
};
...
// Creating a dog with the parameters passed in
const createDog = (req, res) => {
  // Checking parameters
  if (!req.body.name || !req.body.breed || !req.body.age) {
    return res.status(400).json({ error: 'A name, breed, and age are required when creating a dog!' });
  }

  // Creating a JSON to hold the new info
  const tempDog = { name: req.body.name, breed: req.body.breed, age: req.body.age };

  // Making the dog with the passed in data
  const newDog = new Dog(tempDog);

  // Saving it to the database
  const addingPromise = newDog.save();

  // After the promise has been resolved, return the info from the dog
  addingPromise.then(() => {
    res.json({ name: newDog.name, breed: newDog.breed, age: newDog.age });
  });

  // Error checking
  addingPromise.catch((err) => res.json(err));
  return res;
};

// Searching for a dog, and updating the age if found
const findDog = (req, res) => {
  // If there is no name passed in, don't search
  if (!req.body.name) {
    return res.json({ error: 'A name is required to search' });
  }

  // Using the method created earlier to find the dog with the passed in name
  Dog.findByName(req.body.name, (err, doc) => {
    // Handling errors
    if (err) return res.json({ err });

    // Handling if there was no dog found with that name
    if (!doc) return res.json({ error: 'No dog was found by that name' });

    const foundDog = doc;
    // Updating the age of the found dog
    foundDog.age++;

    // Making a promise for saving it
    const updatePromise = foundDog.save();

    // Sending back the
    updatePromise.then(() => res.json(
      { name: foundDog.name, breed: foundDog.breed, age: foundDog.age },
    ));
